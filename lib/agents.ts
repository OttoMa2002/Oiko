...
严格要求（违反则系统无法渲染预览）：
1. 输出**完整可运行**的 HTML 文档：必须以 \`<!DOCTYPE html>\` 开头，以 \`</html>\` 结尾。
2. **每一轮迭代都必须重新输出完整文档**，即使用户只要求"小改"、"加一行"、"修改某个值"——也要重出整份 HTML。**绝不**返回 diff、patch、补丁、片段、或"只列出修改部分"。
3. 不要在 HTML 前后加任何解释文字、客套话或 markdown 围栏。**不写** \`\`\`html、不写"我已修改..."、不写"请看下方代码"。直接以 \`<!DOCTYPE html>\` 起手，以 \`</html>\` 收尾。
4. 使用 Tailwind CSS CDN（\`<script src="https://cdn.tailwindcss.com"></script>\`）。
5. 响应式设计，移动端适配。
6. 总行数不超过 500 行。
7. 不使用外部 JS 框架。
8. 内联所有样式，不引用外部 CSS 文件（Tailwind CDN 除外）。
9. 图片使用 placeholder（https://placehold.co/）。
...
重要：直接输出 JSON 对象，从 \`{\` 开始到 \`}\` 结束。不要在 JSON 前后添加任何解释文字、客套话或 markdown 围栏（不要写 \`\`\`json）。每段 feedback 控制在 80 字以内，improvements 给 3–5 条。`;

export const AGENT_MAX_TOKENS: Record<AgentStage, number> = {
  research: 500,
  architecture: 500,
  // Bumped from 4096 → 8192 to roughly double code-agent output capacity.
  // At 4096 a moderately complex single-page (~110 lines of dense inline-style
  // HTML, e.g. a 6-section resume) saturates the budget on first generation,
  // so any iteration that re-emits the full document gets truncated mid-write.
  // 8192 gives ~2x headroom and still completes within Vercel Hobby's 60s
  // function timeout (typical 8K-token code response is 30–45s).
  // Hard ceiling here, not a roadmap waypoint: the real fix for arbitrarily
  // large projects is diff-style editing (the v0 / bolt.new approach), out
